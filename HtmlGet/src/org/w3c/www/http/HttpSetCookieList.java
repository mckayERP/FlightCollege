...
// $Id: HttpSetCookieList.java,v 1.1.2.1 2003/05/28 22:58:49 owenc Exp $
...
public class HttpSetCookieList extends BasicValue {
    Vector<HttpSetCookie> setcookies = null;
    Vector<HttpBuffer> raws = null;
    ...
    {
	checkByteValues();
	out.write(raws.elementAt(cookie).getByteCopy());
    }
	...
	raws = new Vector<HttpBuffer>(len);
	for (int i = 0 ; i < len ; i++) {
	    HttpBuffer buf = new HttpBuffer();
	    HttpSetCookie sc = setcookies.elementAt(i);
	...
	while (HttpParser.nextItem(raw, cl) >= 0) {
		...
		if (inExpireItem) {
		    ...
		    if ( item.equals("secure") | item.equals("httponly")  ) {
	...
	for (int i = 0 ; i < setcookies.size() ; i++) {
	    HttpSetCookie setcookie = setcookies.elementAt(i);
    ...
    HttpSetCookieList() {
	this.isValid    = false;
	this.setcookies = new Vector<HttpSetCookie>(2);
    }
