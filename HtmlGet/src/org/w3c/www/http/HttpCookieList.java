// HttpCookieList.java
// $Id: HttpCookieList.java,v 1.1.2.1 2003/05/28 22:58:48 owenc Exp $
...
public class HttpCookieList extends BasicValue {
    Vector<HttpCookie> cookies = null;
	...
	int sz = cookies.size();
	for (int i = 0 ; i < sz ; i++) {
	    HttpCookie c = cookies.elementAt(i);
	    if (i != 0) {
		buf.append((byte) ';');
		buf.append((byte) ' ');
	    }
	...
	this.isValid = true;
	this.cookies = new Vector<HttpCookie>(8);
	if ( c != null ) {
	...
	this.isValid = false;
	this.cookies = new Vector<HttpCookie>(2);
    }
