...
public class FinReport extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("ReportDate"))
				p_ReportDate = para[i].getParameterAsTimestamp();
			else
	...
	private void setPeriods()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (p_ReportDate != null 
						&& p_ReportDate.compareTo(rs.getTimestamp(3))>=0 
						&& p_ReportDate.compareTo(rs.getTimestamp(4))<=0)
				{
					endDate = p_ReportDate;
				}
				FinReportPeriod frp = new FinReportPeriod (rs.getInt(1), rs.getString(2),
						rs.getTimestamp(3), endDate, rs.getTimestamp(5));
				list.add(frp);
	...
	private void insertLine (int line)
	{
		...
		for (int col = 0; col < m_columns.length; col++)
		{
			...
			if (m_lines[line].getPAPeriodType() != null)			//	line amount type overwrites column
			{
				...
				{
					select.append("= ").append(DB.TO_DATE(p_ReportDate));
				}
				else
				{
