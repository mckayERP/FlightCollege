...
public class DunningRunCreate extends SvrProcess
{
	...
	//	log.info(sql);
		...
		sql2 = "SELECT COUNT(*), COALESCE(DAYSBETWEEN(MAX(dr2.DunningDate), MAX(dr.DunningDate)),0), dl.C_DunningLevel_ID, dl.DaysAfterDue "		
			...
			+ "GROUP BY dl.C_DunningLevel_ID, dl.DaysAfterDue "
			+ "ORDER BY dl.DaysAfterDue DESC, dl.C_DunningLevel_ID";
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if (rs2.next())
				{
					...
					LastLevel_ID = rs2.getInt(3);  // The query is ordered to return the largest level dunned in the first record.
					LastDaysAfterDue = rs2.getInt(4);
				}
				rs2.close();
				//	SubQuery
				
				// Ensure that Daysbetween Dunning is enforced within a dunning level. 
				...
				if (DaysBetweenDunning != 0  	// DaysBetweenDunning = 0 means don't test it. 
					&& TimesDunned > 0 			// Only interested in items that have been dunned
					&& LastLevel_ID != 0 		// Where a valid level exists
					&& (	(LastLevel_ID == level.getC_DunningLevel_ID() 	// and the level is the same (a repeat)
				...
				if (DaysAfterLast < DaysBetweenDunning 	&& level.isShowAllDue ()) 
	...
	private int addPayments(MDunningLevel level)
	{
		...
		// If it is not a statement we will add lines only if InvoiceLines exists at the same level.
		// We do not want to dunn for money we owe the customer, and we don't want the payments to
		// repeat on other dunning levels.
		if (!level.isStatement())
			sql += " AND C_BPartner_ID IN (SELECT C_BPartner_ID FROM C_DunningRunEntry "
				   + "WHERE C_DunningRun_ID=" + m_run.get_ID () + " "
				   + "AND C_DunningLevel_ID=" + level.getC_DunningLevel_ID() + ")";
