...
public class ImportBankStatement extends SvrProcess implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		//  Fire validator prior to import to clean up imported data, if required
		...
		if (no != 0)
		 ...
		 //	Set Charge based on name
		sql = new StringBuffer("UPDATE I_BankStatement i "
		 	+ "SET C_Charge_ID=(SELECT C_Charge_ID FROM C_Charge ch "
		 	+ "WHERE i.ChargeAmt <> 0 AND i.ChargeName != '' AND ch.name=i.ChargeName ");
		sql.append(clientCheck).append(")").append(" WHERE i.C_Charge_ID IS NULL AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
			log.info("ChargeName=" + no);
		...
		if (no != 0)
		 ...
		 //	Set Payment based on document number match or check no match
		sql = new StringBuffer("UPDATE I_BankStatement i "
		 	+ "SET C_Payment_ID=(SELECT C_Payment_ID FROM C_Payment p "
		 	+ "WHERE i.paymentdocumentno != '' AND p.documentno=i.paymentdocumentno "
		 	+ " AND p.docstatus not in ('VO', 'RE') "
 			+ " AND p.isreconciled<>'Y'");
		sql.append(clientCheck).append(")").append(" WHERE i.C_Payment_ID IS NULL AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no > 0)
		{
			log.info("Payment=" + no);
			 //	Set BP to match Payment
			sql = new StringBuffer("UPDATE I_BankStatement i "
			 	+ "SET C_BPartner_ID=(SELECT C_BPartner_ID FROM C_Payment p "
			 	+ "WHERE p.C_Payment_ID=i.C_Payment_ID");
			sql.append(clientCheck).append(")").append(" WHERE i.C_BPartner_ID IS NULL AND I_IsImported<>'Y'").append(clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());			
			log.info("Bpartner=" + no);

			//	Set Invoice to match Payment
			sql = new StringBuffer("UPDATE I_BankStatement i "
			 	+ "SET C_Invoice_ID=(SELECT C_Invoice_ID FROM C_Payment p "
			 	+ "WHERE p.C_Payment_ID=i.C_Payment_ID");
			sql.append(clientCheck).append(")").append(" WHERE i.C_Invoice_ID IS NULL AND I_IsImported<>'Y'").append(clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());			
			log.info("Invoice=" + no);
		}
		else if (no < 0) //  There was an error - likely duplicate document no?
		{
			sql = new StringBuffer("UPDATE I_BankStatement "
					+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'Err=Duplicate payments identified based on document no?, ' "
					+ "WHERE I_BankStatement_ID IN "
						+ "(SELECT I_BankStatement_ID"
						+ " FROM I_BankStatement i"
						+ " INNER JOIN (SELECT documentNo, COUNT(C_Payment_ID) AS NumberDocs from C_Payment"
						+ " 			WHERE docstatus not in ('VO', 'RE')"
						+ " 			AND isreconciled<>'Y'" + clientCheck
						+ "				GROUP BY p.documentNo) c ON (c.DocumentNo = i.paymentdocumentno)"
						+ " WHERE i.paymentdocumentno IS NOT NULL AND c.NumberDocs > 1) ")
					.append(clientCheck);
				no = DB.executeUpdate(sql.toString(), get_TrxName());
				if (no != 0)
					log.info("Possible duplicate payments based on doc number? =" + no);
		}

		//  Check for other errors such as voided or reversed payments or previously reconciled payments
		sql = new StringBuffer("UPDATE I_BankStatement "
				+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'Err=Payment with that Document Number is voided or reversed., ' "
				+ "WHERE I_BankStatement_ID IN "
					+ "(SELECT I_BankStatement_ID"
					+ " FROM I_BankStatement i"
					+ " INNER JOIN (SELECT DocumentNo FROM C_Payment p "
					+ "             WHERE p.docstatus in ('VO', 'RE')" + clientCheck + ") c ON (c.DocumentNo = i.paymentdocumentno)"
					+ " WHERE i.paymentdocumentno IS NOT NULL)")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
			log.info("Reconciling voided or reversed payment on doc number? =" + no);

		sql = new StringBuffer("UPDATE I_BankStatement "
				+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'Err=Payment with that Document Number is already reconciled., ' "
				+ "WHERE I_BankStatement_ID IN "
					+ "(SELECT I_BankStatement_ID"
					+ " FROM I_BankStatement i"
					+ " INNER JOIN (SELECT DocumentNo FROM C_Payment p "
					+ "             WHERE p.isReconciled = 'Y'" + clientCheck + ") c ON (c.DocumentNo = i.paymentdocumentno)"
					+ " WHERE i.paymentdocumentno IS NOT NULL)")
				.append(clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());
			if (no != 0)
				log.info("Payment is already reconciled =" + no);
		...
		//  Fire validator again prior to import
