...
public class MColorLookup extends Lookup implements Serializable {

	private static final KeyColorPair NO_INSTANCE = new KeyColorPair (0, "", null);
	private static final KeyNamePair NO_INSTANCE_NAMEPAIR = new KeyNamePair (0, "");
	private Properties m_ctx;

	public MColorLookup(Properties ctx, int windowNo) {
		super(DisplayType.Color, windowNo);
		m_ctx = ctx;
	}
	 ...
	 *  @param TabNo tab no
	 */
	public MColorLookup (MLookupInfo info, int TabNo)
	{
		super(info.DisplayType, info.WindowNo);
		
		if (DisplayType.Color != info.DisplayType)
			throw new IllegalArgumentException("Incorrect display type. Expecting 'Color'");
	...
	public String getDisplay(Object key) {
		...
		//
		ColorPair display = getColorPair (key);
		if (display == null)
			return "<" + key.toString() + ">";
	...
	public ColorPair getColorPair(Object key) {
		...
		//	try cache
		ColorPair retValue = (ColorPair)m_lookup.get(key);
		if (retValue != null)
			return retValue;
		...
		if (m_loader != null && m_loader.isAlive())
		{
			...
			loadComplete();
			//	is most current
			retValue = (ColorPair)m_lookup.get(key);
			if (retValue != null)
				return retValue;
		}
		...
		//	log.finest( m_lookup.keySet().toString(), "ContainsKey = " + m_lookup.containsKey(key));
			...
			return getDirectColor(key, false, true);		//	cache locally    
		}
		...
		return getDirectColor(key, false, cacheLocal);	//	do NOT cache	
	}
	...
	public NamePair get (Object key)
	{
		...
		if (key instanceof Integer)
			AD_Color_ID = ((Integer)key).intValue();
		else
		{
			try
			{
				AD_Color_ID = Integer.parseInt(key.toString());
			}
			catch (Exception e)
			{
		...
		if (AD_Color_ID == 0)
			return NO_INSTANCE_NAMEPAIR;
		//
		
		String name = getColorPair(key).getName();
				
		return new KeyNamePair (AD_Color_ID, name);
	...
	{
		if (obj instanceof MColorLookup)
		{
			MColorLookup ll = (MColorLookup)obj;
	 ...
	 *  @return value
	 */
	public ColorPair getDirectColor (Object key, boolean saveInCache, boolean cacheLocal)
	{
		//	Nothing to query
		...
		if (key.equals(m_directNullKey))
			return null;
		//
		ColorPair directValue = null;
		if (m_lookupDirect != null)		//	Lookup cache
		{
			directValue = (ColorPair) m_lookupDirect.get(key);
			if (directValue != null)
				return directValue;
		}
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if (isNumber)
				{
					...
					KeyColorPair p = new KeyColorPair(keyValue, name, c);
				...
				{
					// Shouldn't happen
					log.severe("Lookup found value not key?");
		...
		public void run()
		{
			...
			try
			{
				...
				while (rs.next())
				{
					...
					{
						int key = rs.getInt(1);
						KeyColorPair p = new KeyColorPair(key, name, c);
						m_lookup.put(new Integer(key), p);
					}
					else
					{
						log.severe("Loader found a value instead of number(key)?");
//						String value = rs.getString(2);
//						ValueNamePair p = new ValueNamePair(value, name);
//						m_lookup.put(value, p);
					}
