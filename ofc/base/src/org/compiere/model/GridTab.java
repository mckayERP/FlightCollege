 ...
 */
public class GridTab implements DataStatusListener, TableModelListener, Evaluatee, Serializable
{
	/**
	 * 
	 */
	...
	/** A class to provide data from other rows as an Evaluatee interface.
	 *  This class is used in conditional formating of a table where the
	 *  fields being formatted are not in the currently selected row.  The
	 *  conditional formating requires same-row information.<br>
	 *  To use this class, call the constructor {@link #GetRowData(int row)}
	 *  with the row of interest and pass the instance as the Evaluatee to
	 *  the EvaluateLogic functions.
	 *  
	 * @author mckayERP
	 *
	 */
	public class RowData implements Evaluatee
	{

		private int m_row = m_currentRow;
		...
		public String get_ValueAsString(String columnName) {
			...
			if (m_row == m_currentRow)
				return GridTab.this.get_ValueAsString(columnName);
			
			Object value = getValue (m_row, columnName);
			return value == null ? "" : value.toString();
		}
		
	}
	
	/** Create an instance of the RowData Evaluatee model
	 * 
	 * @param row
	 * @return a RowData model set for the given row.
	 */
	public RowData createRowData(int row) 
	{
	...
	/** Map of ColumnName of source field (key) and the conditional formatting 
	 * dependent field (value) 
	 * */
	private MultiMap<String,GridField>	m_condDepOnField = new MultiMap<String,GridField>();
	...
	private boolean loadFields()
	{
		...
		for (int f = 0; f < m_vo.getFields().size(); f++)
		{
			...
			if (voF != null)
			{
				...
				//  List of ColumnNames, the conditional formating of this field is dependent on
				ArrayList<String> condlist = field.getConditionalDependentOn();
				for (int i = 0; i < condlist.size(); i++)
					m_condDepOnField.put(condlist.get(i), field);   //  ColumnName, Field
	...
	public void query (boolean onlyCurrentRows, int onlyCurrentDays, int maxRows)
	{
		...
		if (isDetail())
		{
			...
			else
			{
				...
				else
				{
					...
					if (where.length() != 0)
						where.append(" AND ");
					// Check for virtual column used as link - replace the name with the sql
					GridField lcField = this.getField(lc);
					if (lcField.isVirtualColumn())
					{
						where.append("(").append(lcField.getColumnSQL(false)).append(")=");
					}
					else
					{
		...
		for (int row = 0; row <= m_mTable.getRowCount(); row++)
		{
			for (int col=0; col < m_mTable.getFieldCount(); col++)
			{
				setConditionalFormat(row, col);
			}
		}
...
//					value = mField.getDefault();
				...
				// Moved to setValueAndUpdateContext() - still need to wipe context if values are null
//				if (mField.isKey())
...
//		if (m_currentRow >= 0)
//		{
//			for (int col=0; col < m_mTable.getColumnCount(); col++)
//			{
//				setConditionalFormat(m_currentRow, col);
//			} 
//		}
	...
	public String processFieldChange (GridField changedField)
	{
		...
		processConditionalDependencies(changedField);
	...
	/**
	 *  Evaluate Conditional Formating Dependencies
	 *  @param changedField changed field
	 */
	private void processConditionalDependencies (GridField changedField)
	{
		String columnName = changedField.getColumnName();
	//	log.trace(log.l4_Data, "Changed Column", columnName);

		//  when column name is not in list of DependentOn fields - fini
		if (!hasConditionalDependants(columnName))
			return;

		//  Get dependent MFields (will be referenced in Conditional Logic)
		ArrayList<GridField> list = getConditionalDependantFields(columnName);
		for (int i = 0; i < list.size(); i++)
		{
			GridField dependentField = (GridField)list.get(i);
			...
			// Test the current row.  Can use "this" as the Evaluatee - its limited to 
			// the current row.
			dependentField.testAndSetConditionalFormat(this, this.getCurrentRow());
	...
	public void switchRows(int from, int to, int sortColumn, boolean ascending) {
		...
		if (!DisplayType.isNumeric(m_mTable.getField(lineCol).getDisplayType()))
		{
		...
		// Not sure this is a good idea - better would be to ensure Line and SeqNo fields are
		// filled by default.  The following is in case the lineCol is null - something that 
		// would happen if the column was added to an existing table with data or the database 
		// isn't configured to set the value as default when its new.
		Integer max = new Integer(0);
		boolean sort = true;
		if (lineNoCurrentRow == null || lineNoNextRow == null)
		{
			String tableName = m_mTable.getTableName();
			String colName = m_mTable.getColumnName(lineCol);
			String where = this.getWhereExtended();
			max = new Query(Env.getCtx(), tableName, where, null)
						.setClient_ID()
						.aggregate(colName, Query.AGGREGATE_MAX).intValue() + 10;
			sort = false;
		}
		if (lineNoCurrentRow == null)
		{
			if (to < from)
			{
				if (lineNoNextRow == null)
					lineNoCurrentRow = max + 10;
				else
					lineNoCurrentRow = max;
			}
			else
			{
				lineNoCurrentRow = max;
		...
		if (lineNoNextRow == null)
		{
			if (lineNoCurrentRow.compareTo(max) != 0)
				lineNoNextRow = max;
			else
			{
				if (to < from)
				{
					lineNoNextRow = lineNoCurrentRow - 10;
				}
				else
				{
					lineNoNextRow = lineNoCurrentRow + 10;
	...
	public void setConditionalFormat(int row, int col) {
		
		GridField gridField = getField(col);
		if (gridField == null)
			return;
		
		if (!gridField.isConditionalFormat() || !gridField.isDisplayed())
			return;
		
		// This sets the conditional formatting for this col in the 
		// current row.
		log.fine("GridTab CurrentRow " + this.getCurrentRow() + " Row " + row + " Col " + col);
		Evaluatee rowData = this.createRowData(row);
		gridField.testAndSetConditionalFormat(rowData, row);
	...
	public void tableChanged(TableModelEvent e) {
		...
		if (e.getType() == TableModelEvent.UPDATE || e.getType() == TableModelEvent.INSERT)
		{
			...
			int lastRow = Math.min(e.getLastRow(), this.getRowCount());
			
			for (int row = firstRow; row <= lastRow; row++)
			{
				if (e.getColumn() == TableModelEvent.ALL_COLUMNS)
				{
					for (int col=0; col < this.getFieldCount(); col++)
					{
//						setConditionalFormat(row, col);
					}
				}
				...
				{
//					setConditionalFormat(row, e.getColumn());
				}
			}
		}
	}
	 ...
	 *  @return true if column has dependent
	 */
	public boolean hasConditionalDependants (String columnName)
	{
	//	m_depOnField.printToLog();
		return m_condDepOnField.containsKey(columnName);
	}   //  isDependentOn

	/**
	 *  Get dependents fields of the conditional formating of columnName
	 *  @param columnName column name
	 *  @return ArrayList with GridFields dependent on columnName
	 */
	public ArrayList<GridField> getConditionalDependantFields (String columnName)
	{
		return m_condDepOnField.getValues(columnName);
	}   //  getDependentFields
