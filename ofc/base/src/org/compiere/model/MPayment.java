	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//	We need a BPartner
		else if (getC_BPartner_ID() == 0 && !isCashTrx() && !isManualDeposit())
		{
	...
	private boolean isManualDeposit() {
	...
	public ValueNamePair[] getCreditCards (BigDecimal amt)
	{
		try
		{
			...
			for (int i = 0; i < m_mPaymentProcessors.length; i++)
			{
				...
				if (m_mPaymentProcessors[i].isAcceptATM ())
					map.put (CREDITCARDTYPE_ATM, getCreditCardPair (CREDITCARDTYPE_ATM));
