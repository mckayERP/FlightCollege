...
public final class MLookup extends Lookup implements Serializable
{
	...
	public MLookup (MLookupInfo info, int TabNo)
	{
		...
		// Set the MPAttributeLookup
		if(m_info.DisplayType == DisplayType.DataPoint) {
			setMDataPointLookup(new MDataPointLookup(Env.getCtx(), this.getWindowNo()));
		}

		// Set the MColorLookup
//		if(m_info.DisplayType == DisplayType.Color) {
//			setMColorLookup(new MColorLookup(Env.getCtx(), this.getWindowNo()));
//		}
	...
	/** MDataPointLookup model */
	private MDataPointLookup	m_DataPointLookup = null;
	...
	public String getDisplay (Object key)
	{
		...
		// DataSetInstances use a different display approach
		if(this.getDisplayType() == DisplayType.DataPoint && getMDataPointLookup() != null) {
			return getMDataPointLookup().getDisplay(key);
		}
	...
	/**
	 * @return the m_DataPointLookup
	 */
	public MDataPointLookup getMDataPointLookup() {
		return m_DataPointLookup;
	}

	/**
	 * @param dataPointLookup the MDataPointLookup to set
	 */
	public void setMDataPointLookup(MDataPointLookup dataPointLookup) {
		this.m_DataPointLookup = dataPointLookup;
	}
