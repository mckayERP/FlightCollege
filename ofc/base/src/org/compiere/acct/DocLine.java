...
{	
	...
	public BigDecimal getProductCosts (MAcctSchema as, int AD_Org_ID, boolean zeroCostsOK, String whereClause)
	{
		...
		MProduct p = getProduct();
		M_AttributeSet_ID = p.getM_AttributeSet_ID();
		
		if (M_AttributeSet_ID != 0)
		{
			MAttributeSet mas = p.getAttributeSet();
			oneLinePerASI = mas.isInstanceAttribute();
		}
		if (oneLinePerASI)  // The ASI field will be valid
		{
			M_AttributeSetInstance_ID = getM_AttributeSetInstance_ID();
		}
		else // Subordinate MA records should exist
		{
			//  Get the set of subordinate MA records and take the ASI of the first one - assuming all ASIs would 
			//  have the same cost.
			MInventoryLineMA[] ma_lines = MInventoryLineMA.get(Env.getCtx(), get_ID(), p_po.get_TrxName());
			if( ma_lines.length > 0 )
				M_AttributeSetInstance_ID = ma_lines[0].getM_AttributeSetInstance_ID();
			else  // No subordinate lines, use the main record.
				M_AttributeSetInstance_ID = getM_AttributeSetInstance_ID();
		}
		if (whereClause != null)
		{
			MCostDetail cd = MCostDetail.get (Env.getCtx(), whereClause, 
					get_ID(), M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), p_po.get_TrxName());
			if (cd != null)
				return cd.getAmt();
		}
