	...
	implements ActionListener
{
	...
	/**
	 *  Open a dialog with the System Color
	 *  @param owner owner
	 *  @param ad_color_id or zero for not known
	 *  @param the initial CompiereColor or null .
	 *  @return ColorEditor 
	 */
	public static ColorEditor showDialog (Frame owner, int ad_color_id, CompiereColor cc, boolean saveSystemColor)
	{	
		return new ColorEditor (owner, ad_color_id, cc, saveSystemColor);
	 ...
	 */
	public ColorEditor (Frame owner, MColor mcolor)
	{
		super(owner, "", true);
		if (mcolor == null)
			mcolor = new MColor(Env.getCtx(), 0, null);
		
		m_AD_Color_ID = mcolor.getAD_Color_ID();
	
		colorName.setValue(mcolor.getName());
		init (mcolor.getAdempiereColor());
	 ...
	 *  Create AdempiereColor Dialog with color
	 *  @param owner owner
	 *  @param ad_color_id the id of an existing color or zero for a new one
	 *  @param the Compiere Color to set initially or null
	 */
	public ColorEditor (Frame owner, int ad_color_id, CompiereColor cc, boolean saveSystemColor)
	{
		super(owner, "", true);
		
		// ad_color_id could be zero - selecting a new color perhaps
		m_saveSystemColor  = saveSystemColor;
		m_mColor = new MColor(Env.getCtx(), ad_color_id, null);
		m_AD_Color_ID = m_mColor.getAD_Color_ID();  // Will be zero if not found.
		...
		if (m_AD_Color_ID <= 0 && cc == null)
		{
			// Set to the defaults
			cc = new CompiereColor();
			m_mColor.setAdempiereColor(cc);
		}
		else if (cc != null) 
		{
			// cc != null - use it as the defined color
			m_mColor.setAdempiereColor(cc);
		}
		// By default if m_AD_Color_ID > 0 and cc== null
	...
	private CTextField colorName = new CTextField(30);
	...
	private void jbInit() throws Exception
	{
		...
		colorNameLabel.setText(res.getString("Color Name"));
		...
		northPanel.add(lineColor,   new GridBagConstraints(0, 8, 1, 1, 0.0, 0.0
			,GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 0, 0));
		northPanel.add(colorNameLabel,   new GridBagConstraints(0, 8, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 0, 0));
		northPanel.add(colorName,   new GridBagConstraints(1, 8, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 5), 0, 0));
	}   //  jbInit
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (e.getSource() == bOK)
		{
			if (colorName.getValue() == null || ((String) colorName.getValue()).isEmpty())
			{
				return;
			}
		...
		//  Lines
			...
			m_cc.setColorName(colorName.getText());
		}
		catch (Exception ee)
		{
		}
	...
	private void saveSelection() {
		...
		// If the confirm button is clicked when the
		// focus is in the Color Name field, the color name
		// may not be set.
		if (colorName.getDisplay() != null)
			m_cc.setColorName(colorName.getDisplay());
		
		if (m_saveSystemColor)
		{
			m_mColor.setAdempiereColor(m_cc);
			m_mColor.saveEx();  // will set the ID
	...
	private void updateFields()
	{
		...
		//  Name
		colorName.setText(m_cc.getColorName());
