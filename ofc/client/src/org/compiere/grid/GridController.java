	...
	GridField mField = m_mTab.getField(col);
		if (mField != null
			&& (mField.getCallout().length() > 0 
				|| m_mTab.hasDependants(mField.getColumnName())
				|| m_mTab.hasConditionalDependants(mField.getColumnName())))
		{
		...
		// Finally, set the conditional formats based on the new data
		m_mTab.setConditionalFormat(e.getCurrentRow(), e.getChangedColumn());
			...
			if (columnName != null && columnName.length() > 0)
			{
				...
				if (mField != null)
				{
					if (mField.isDisplayed(true))		//  check context
					{
						...
						// Special case of the Color field in the System Color window (AD_Color)
						if (comp instanceof VColor && m_mTab.get_TableName().equals(MColor.Table_Name) && mField.isKey())
						{
							((VColor) comp).reload();
						}
