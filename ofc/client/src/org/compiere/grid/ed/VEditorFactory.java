...
public class VEditorFactory
{
	...
	public static VEditor getEditor (GridTab mTab, GridField mField, boolean tableEditor)
	{
		...
		//	Lookup
				...
				&& displayType != DisplayType.DataPoint
				&& displayType != DisplayType.Color) || displayType == DisplayType.ID)
		{
		...
		{
			VColor color = new VColor (mTab, mandatory, readOnly, updateable, 0,
					mField.getLookup(), false);
		...
		else if (displayType == DisplayType.Chart )
		{
			...
			chart.setName(columnName);
			chart.setField(mField);
			editor = chart;
		}

		//  Data Point
		else if (displayType == DisplayType.DataPoint)
		{
			VDataPoint data = new VDataPoint (mTab, mandatory, readOnly, updateable, WindowNo,
				(MLookup)mField.getLookup(), false);
			data.setName(columnName);
			data.setField (mField);
			editor = data;
		}
